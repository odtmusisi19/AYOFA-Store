...
if(!isset($_SESSION["login"])){
  ...
  if(isset($_POST["ubah"])){
    ...
    $negara           = $_POST["negara"];
     ...
     $update = mysqli_query($connection, "UPDATE tabel_pembayaran SET alamat='$alamat', kota='$kota', kode_pos='$kodepos', negara='$negara'");
    if ($update) {
       header("Location: cart.php");
    }
  }

  $banyak_alamat = mysqli_query($connection, "SELECT * FROM tabel_pembayaran");
   $rowcount = mysqli_num_rows($banyak_alamat);

  // UPDATE `tabel_pembayaran` SET `nama_pelanggan` = 'ogidarmatena' WHERE `tabel_pembayaran`.`id` = 1; 
            ...
            <div class="row justify-content-center">
            	<div class="col-md-6 border mt-4 shadow-sm white">
            		...
            		<form action="#" class="signin-form" method="post">
                            ...
                            <input value="<?= $bayar['alamat'] ?>" style="border: 1px solid black;" type="alamat" id="alamat" name="alamat" class="form-control" placeholder="alamat" required>
                        </div>
                        <div class="form-group mb-3">
                            <label class="label" for="kota">Kota</label>
                            <input value="<?= $bayar['kota'] ?>" style="border: 1px solid black;" type="kota" id="kota" name="kota" class="form-control" placeholder="Kota" required>
                        </div>
                    <div class="form-group mb-3">
                        <label class="label" for="kodepos">Kode Pos</label>
                      <input value="<?= $bayar['kode_pos'] ?>" style="border: 1px solid black;" type="kodepos" id="kodepos" name="kodepos" class="form-control" placeholder="Kode Pos" required>
                    </div>
                    <div class="form-group mb-3">
                        <label class="label" for="negara">Negara</label>
                      <input value="<?= $bayar['negara'] ?>" style="border: 1px solid black;" type="negara" id="negara" name="negara" class="form-control" placeholder="Negara" required>
                    </div>
                    <div class="form-group">
                        <button name="ubah" type="submit" class="form-control btn rounded submit px-3" style="background-color: #00A5C4">Ubah</button>      
